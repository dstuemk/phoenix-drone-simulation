...
class ProximalPolicyOptimizationAlgorithm(iwpg.IWPGAlgorithm):
    ...
    def compute_loss_pi(self, data) -> tuple:
        ...
        ratio = torch.exp(_log_p - data['log_p']) * data['mask']
        clip_adv = data['adv'] * torch.clamp(
            ratio,
            1 - self.clip_ratio,
            1 + self.clip_ratio)
        loss_pi = -(torch.min(ratio * data['adv'], clip_adv) * data['mask']).mean()
        loss_pi /= data['mask'].mean()
        ...
        pi_info = dict(kl=approx_kl, ent=ent, 
            ratio=(ratio.mean() / data['mask'].mean()).item())
